...
function ensureCss() {
  ...
  css.textContent = [
    ...
    '.ai-icon-btn{width:28px;height:28px;padding:0;border:none;background:transparent;color:#6b7280;cursor:pointer;border-radius:4px;transition:all .15s;display:flex;align-items:center;justify-content:center}',
    '.ai-icon-btn:hover{color:#374151;background:rgba(0,0,0,.06)}',
    '.ai-icon-btn svg{width:16px;height:16px;stroke-width:2}',
    '#ai-assist-win.dark .ai-icon-btn{color:#9ca3af}',
    ...
    // 新增：输入框区域样式
    '.ai-input-wrap{position:relative;width:100%;background:#fff;border:1px solid #e5e7eb;border-radius:10px;overflow:hidden;display:flex;flex-direction:column}',
    '.ai-input-wrap textarea{width:100%;min-height:100px;max-height:180px;overflow-y:auto;background:transparent;border:none;color:#0f172a;padding:10px 40px 10px 10px;resize:none;font-family:inherit;font-size:13px;box-sizing:border-box;outline:none}',
    '.ai-input-wrap:focus-within{border-color:#3b82f6}',
    '.ai-quick-action-wrap{flex-shrink:0;padding:8px 10px;display:flex;align-items:center;gap:4px;background:#fff;border-top:1px solid #f0f0f0}',
    ...
    '#ai-assist-win.dark .ai-quick-action-wrap{background:#1a1b1e;border-top-color:#2a2b2f}',
...
async function mountWindow(context){
  ...
  el.innerHTML = [
    ...
    '    <button id="ai-btn-history" class="ai-icon-btn" title="' + aiText('会话历史', 'History') + '"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor"><path d="M3 12a9 9 0 1 0 9-9 9.75 9.75 0 0 0-6.74 2.74L3 8"/><path d="M3 3v5h5"/><path d="M12 7v5l4 2"/></svg></button>',
    '    <button id="ai-s-new" class="ai-icon-btn" title="' + aiText('新建会话', 'New session') + '"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor"><path d="M12 5v14M5 12h14"/></svg></button>',
    '    <div class="ai-more-menu-wrap">',
    '      <button id="ai-btn-more" class="ai-icon-btn" title="' + aiText('更多', 'More') + '"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor"><circle cx="12" cy="12" r="1"/><circle cx="12" cy="5" r="1"/><circle cx="12" cy="19" r="1"/></svg></button>',
    ...
    '      </div>',
    '    </div>',
    '    <button id="ai-btn-close" class="ai-icon-btn" title="' + aiText('关闭', 'Close') + '"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor"><path d="M18 6 6 18M6 6l12 12"/></svg></button>',
    '  </div>',
    '</div>',
    // 会话历史下拉面板
  ...
  if (context.addRibbonButton) {
    try {
      context.addRibbonButton({
        ...
        title: aiText('AI 助手', 'AI Assistant'),
        onClick: async () => { await toggleWindow(context) }
      })
    } catch (e) { console.error('[AI] addRibbonButton failed', e) }
  }
